 ...
 *  Search: O(logN)
 *
 *  But since it is not guaranteed that the tree is balanced,
 *  creating can take O(N^2), while other operations can take O(N) each.
 */
...
int contains(int val) { return search(root,val);} //helper function
